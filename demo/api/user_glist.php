...
{	
	...
	{ 
       ...
       if ($query[0]['utype'] == 'A')
       {
           $query_guser = $conn->prepare("SELECT u.userid,u.ufname,u.ulname,u.ucontact,u.user_token FROM user u where usts='A' AND user_varified='Yes' AND userid!=1 AND utype!='A' order by u.ufname asc");
...
{	
 //echo "SELECT gid,gname FROM group_table WHERE created_by=".$_REQUEST['UserId'];
