...
if($_REQUEST['userid'] && $_REQUEST['userid']!='')
{
    ...
    {   
        ...
        $query = mysql_query("SELECT userid, ufname, ulname, ucontact, uemail FROM user where userid in (".$u.") AND user_varified='Yes' AND userid!=".$_REQUEST['userid']." AND userid!=1 AND utype!='A' group by userid order by ufname asc",$conn);
