...
if($_REQUEST['UserId']!= '' || $_REQUEST['Type']!= '' || $_REQUEST['FromDate']!= '')
{
 ...
 $ins = "INSERT INTO reminder (type, title, description, location, rem_req, priority, from_date, to_date, created_by, Status, start_date, period, activity) VALUES ('".$Type."','".$Tital."','".$Description."','".$Location."', '".$Reminder_Require."','".$Priority."', '".$FromDate."','".$ToDate."','".$UserId."', 'Pending', '".$StartDate."','".$Int."', 'A')";
...
if($_REQUEST['UserId']!= '' || $_REQUEST['Type']!= '' || $_REQUEST['FromDate']!= '')
{
 ...
 if($_REQUEST['Reminder_Require']=='Y'){ $Reminder_Require=1; }else{ $Reminder_Require=0; }
 $StartDate=date("Y-m-d h:i:s",strtotime($_REQUEST['StartDate']));
 if($_REQUEST['Interval']=='Onces a day'){$Int=24;}
 elseif($_REQUEST['Interval']=='Twice a Day'){$Int=12;}
 elseif($_REQUEST['Interval']=='Thrice a Day'){$Int=8;}
 else{$Int=0;}
