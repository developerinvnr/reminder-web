...
{
 $image = '';
 if(isset($_FILES['chat_img'])){
     $image = 'chat_'.$_REQUEST['UserId'].'_'.date("dmyHis").'.jpg';
    move_uploaded_file($_FILES['chat_img']['tmp_name'],"up_img/".$image);
 }
  $query = mysql_query("insert into reminder_participants_chat(rem_id,chat_rmk,chat_date,chat_img,TeamBy) values(".$_REQUEST['remid'].", '".$_REQUEST['msg']."', '".date("Y-m-d H:i:s")."','".$image."' ,".$_REQUEST['UserId'].")");
